...
def process_embeddings(model, input_ids, screen_position, get_screen=True, adapt_emb=None):
    ...
    if get_screen:
        ...
        # screen_tensor["current"] = input_embedding[:, screen_position["current"], :].detach().clone()
        # screen_tensor["history"] = input_embedding[:, screen_position["history"], :].detach().clone()
...
def llm_process(args, pp_path, poi_path):
    ...
    try:
        ...
        print("Flash Attention enabled: flash_attention_2")
    except (TypeError, ValueError, ImportError) as e:
        print(f"Flash Attention not available, fallback to default attention. Reason: {e}")
        model = AutoModelForCausalLM.from_pretrained(
            model_name,
            torch_dtype=torch.float16,
            local_files_only=False,
    ...
    for iter_count in range(iter_count_start, data_end):
        ...
        ################# iteration for important nodes selection #################
            ...
            if correct:  # only need the true loss grad
                ...
                all_weights_true, benchmark_true = cal_all_weight(screen_tensor=screen_tensor)
                ...
                all_weights_false, benchmark_false = cal_all_weight(screen_tensor=screen_tensor)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--p_rank", type=int, default=2)
